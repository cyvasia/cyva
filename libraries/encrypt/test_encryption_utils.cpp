...
using cyva::encrypt::DInteger;
void test_aes(cyva::encrypt::AesKey k)

{
   cyva::encrypt::AES_encrypt_file("/tmp/test_file.txt","/tmp/test_file.out", k);
   cyva::encrypt::AES_decrypt_file("/tmp/test_file.out","/tmp/test_file.orig",k);

}

void test_el_gamal(cyva::encrypt::AesKey k)
{
   cout<<"Catchpoint 0 \n";
   DInteger pk1 = cyva::encrypt::generate_private_el_gamal_key();
   DInteger pk2 = cyva::encrypt::generate_private_el_gamal_key();
   DInteger pubk1 = cyva::encrypt::get_public_el_gamal_key(pk1);
   DInteger pubk2 = cyva::encrypt::get_public_el_gamal_key(pk2);


   cout <<"pk1 = " << pk1.to_string();
   cout <<"pk2 = " << pk2.to_string();
   cout<<"Catchpoint 1 \n";
   cyva::encrypt::point secret;
   secret.first = DInteger(10000);
   secret.second = DInteger(1000000000);

   cout<<"Catchpoint 2 \n";
   cyva::encrypt::Ciphertext ct1, ct2;
   cyva::encrypt::el_gamal_encrypt(secret, pubk1, ct1);

   cyva::encrypt::point received_secret;
   cyva::encrypt::el_gamal_decrypt(ct1,pk1,received_secret);
   cout<<"Catchpoint 3 \n";
   ...
   cyva::encrypt::DeliveryProof proof(CryptoPP::Integer::One(),CryptoPP::Integer::One(),CryptoPP::Integer::One(),CryptoPP::Integer::One(),CryptoPP::Integer::One());

   cout<<"Catchpoint 4 \n";
   cyva::encrypt::encrypt_with_proof(received_secret, pk1, pubk2, ct1, ct2, proof);
   cout<<"Catchpoint 5 \n";
   cyva::encrypt::point received_secret2;
   cyva::encrypt::el_gamal_decrypt(ct2,pk2,received_secret2);

   cout <<"recovered secret is "<<received_secret.first.to_string()<<" "<<received_secret.second.to_string() <<"\n";

   for (int i=0; i<1; i++)
      bool ret_val = cyva::encrypt::verify_delivery_proof(proof, ct1,ct2,pubk1,pubk2);
   /*if(ret_val)
      cout<< "everything OK!\n";*/

}

void test_shamir(cyva::encrypt::DInteger secret)
{
   cyva::encrypt::ShamirSecret ss(5,9,secret);
   ss.calculate_split();
   cyva::encrypt::point x0 = ss.split[0];
   cyva::encrypt::point x1 = ss.split[1];
   cyva::encrypt::point x2 = ss.split[2];
   cyva::encrypt::point x3 = ss.split[3];
   cyva::encrypt::point x4 = ss.split[6];

   cyva::encrypt::ShamirSecret rs(5,9);
...
void test_custody(){
   ...
   cyva::encrypt::CustodyData cd;
   cyva::encrypt::CustodyProof proof;
...
void test_generator(){
   ...
   element_set_str(gen, _CYVA_GENERATOR_, 10);
