...
// Copyright (c) 2015-2015 CYVA developers
...
CustodyUtils::CustodyUtils() {
   pairing_init_set_str(pairing, _CYVA_PAIRING_PARAM_);

   element_init_G1(generator, pairing);

   element_set_str(generator, _CYVA_GENERATOR_, 10);
}
...
int CustodyUtils::get_u_from_seed(const mpz_t &seedU, element_t out[]) {
   mpz_t seed;
   mpz_t seed_tmp;
   mpz_init(seed_tmp);
...
int CustodyUtils::get_n(std::fstream &file) {
   ...
   int n = length / (CYVA_SIZE_OF_NUMBER_IN_THE_FIELD * CYVA_SECTORS);
   if( length % (CYVA_SIZE_OF_NUMBER_IN_THE_FIELD * CYVA_SECTORS))
      n += 1;
...
inline int CustodyUtils::get_data(std::fstream &file, uint32_t i, char buffer[]) {
   uint64_t position = CYVA_SIZE_OF_NUMBER_IN_THE_FIELD * CYVA_SECTORS * i;

   file.seekg(0, file.end);
   uint64_t realLen = file.tellg();
   file.seekg(std::min(position, realLen), file.beg);

   if( realLen > ((long long) (file.tellg()) + CYVA_SIZE_OF_NUMBER_IN_THE_FIELD * CYVA_SECTORS))
      file.read(buffer, CYVA_SIZE_OF_NUMBER_IN_THE_FIELD * CYVA_SECTORS);
   else {
      if( file.eof())
         memset(buffer, 0, CYVA_SIZE_OF_NUMBER_IN_THE_FIELD * CYVA_SECTORS);
      else {
         int left = realLen - (long long) (file.tellg());
         file.read(buffer, left);
         memset(buffer + left, 0, (CYVA_SIZE_OF_NUMBER_IN_THE_FIELD * CYVA_SECTORS) - left);
      }
   }
   return 0;
}
...
inline int CustodyUtils::get_m(std::fstream &file, uint32_t i, uint32_t j, mpz_t &out) {
   mpz_init2(out, CYVA_SIZE_OF_NUMBER_IN_THE_FIELD * 8);
   uint64_t position = CYVA_SIZE_OF_NUMBER_IN_THE_FIELD * (j + CYVA_SECTORS * i);
   char buffer[CYVA_SIZE_OF_NUMBER_IN_THE_FIELD];
   file.seekg(0, file.end);
   uint64_t realLen = file.tellg();
   if( realLen < position ) { //we are either close or behinf EoF
      memset(buffer, 0, CYVA_SIZE_OF_NUMBER_IN_THE_FIELD);
   } else {
      file.seekg(position, file.beg);
      if( realLen < position + CYVA_SIZE_OF_NUMBER_IN_THE_FIELD ) { //we can't read the whole string
         int left = realLen - (uint64_t) (file.tellg());
         file.read(buffer, left);
         memset(buffer + left, 0, CYVA_SIZE_OF_NUMBER_IN_THE_FIELD - left);
      } else {
         file.read(buffer, CYVA_SIZE_OF_NUMBER_IN_THE_FIELD);
      }
   }
   ...
   memcpy((char *) out->_mp_d, buffer, CYVA_SIZE_OF_NUMBER_IN_THE_FIELD);
   out->_mp_size = CYVA_MP_SIZE_OF_NUMBER_IN_THE_FIELD;
   //mpz_import( out, CYVA_SIZE_OF_NUMBER_IN_THE_FIELD, 1, 1, 1, 0, buffer );
   return 1;
}

int CustodyUtils::get_sigma(uint64_t idx, mpz_t mi[], element_pp_t u_pp[], element_t pk, element_t out[]) {
   element_t temp;
...
CustodyUtils::get_sigmas(std::fstream &file, const unsigned int n, element_t *u, element_t pk, element_t **sigmas) {
   ...
   //start threads
   fc::thread t[CYVA_CUSTODY_THREADS];
   fc::future<int> fut[CYVA_CUSTODY_THREADS];
   fc::future<void> fut_pp[CYVA_CUSTODY_THREADS];

   element_pp_t *u_pp = new element_pp_t[CYVA_SECTORS];
   for( int k = 0; k < CYVA_SECTORS; k++ ) {
      int idx = k % CYVA_CUSTODY_THREADS;
      fut_pp[idx] = t[idx].async([=](){ element_pp_init(u_pp[k], u[k]); return; });
   }
   //wait for the threads...
   for( int k = 0; k < CYVA_CUSTODY_THREADS; ++k )
      fut_pp[k].wait();

//   std::cout<<"get_sigmas: n = "<<n<<" , cycles = " << cycles <<"\n";
   int total_thread_to_wait_for = std::min( n, (const unsigned int) CYVA_CUSTODY_THREADS );

   for( uint64_t i = 0; i < n; i += CYVA_CUSTODY_THREADS ) {
      int iterations = std::min ((uint64_t) CYVA_CUSTODY_THREADS, n - i ); 
      for( int k = 0; k < iterations ; ++k ) {
         uint64_t idx = i + k;

         //we read the file in the main thread...
         char *buffer = new char[(CYVA_SIZE_OF_NUMBER_IN_THE_FIELD * CYVA_SECTORS)];
         get_data(file, idx, buffer);
         //and distribute the tasks
         fut[k] = t[k].async([=]() {
              mpz_t m[CYVA_SECTORS];
              for( int i = 0; i < CYVA_SECTORS; ++i ) {
                 mpz_init2(m[i], CYVA_SIZE_OF_NUMBER_IN_THE_FIELD * 8);
                 //mpz_import is too slow for our purposes - since we don't care about the exact parameters as much as about the uniqueness of the import, let's replace it with memcpy
                 memcpy((char *) m[i]->_mp_d, buffer + i * CYVA_SIZE_OF_NUMBER_IN_THE_FIELD,
                        CYVA_SIZE_OF_NUMBER_IN_THE_FIELD);
                 m[i]->_mp_size = CYVA_MP_SIZE_OF_NUMBER_IN_THE_FIELD;
                 //mpz_import(m[i], CYVA_SIZE_OF_NUMBER_IN_THE_FIELD, 1, 1, 1, 0, buffer + i * CYVA_SIZE_OF_NUMBER_IN_THE_FIELD);
              }
              delete[] buffer;
...
int CustodyUtils::compute_mu(std::fstream &file, unsigned int q, uint64_t indices[], element_t v[], element_t mu[]) {

   for( int j = 0; j < CYVA_SECTORS; j++ ) {
...
int CustodyUtils::verify(element_t sigma, unsigned int q, uint64_t *indices, element_t *v, element_t *u, element_t *mu,
   ...
   for( int i = 0; i < q; i++ ) {
      ...
      memcpy(buf, stemp._hash, (4 * sizeof(uint64_t)));
      element_from_hash(hash, buf, 32);
      element_pow_zn(temp, hash, v[i]); //TODO_CYVA optimize
#ifdef _CUSTODY_STATS
      pow++;
#endif
      if( i ) {
...
int CustodyUtils::unpack_proof(valtype proof, element_t &sigma, element_t **mu) {
   ...
   pointer += CYVA_SIZE_OF_POINT_ON_CURVE_COMPRESSED;

   element_t *ret = new element_t[CYVA_SECTORS];
   for( int i = 0; i < CYVA_SECTORS; i++ ) {
      element_init_Zr(ret[i], pairing);
      element_from_bytes(ret[i], proof.data() + pointer);
      pointer += CYVA_SIZE_OF_NUMBER_IN_THE_FIELD;
   }
   *mu = ret;
   return 0;
}
...
int CustodyUtils::verify_by_miner(const uint32_t &n, const char *u_seed, unsigned char *pubKey, unsigned char sigma[],
   ...
   for( int i = 0; i < CYVA_SECTORS; i++ ) {
      ...
      unsigned char buffer[CYVA_SIZE_OF_MU];
      string_to_bytes(mus[i], buffer, CYVA_SIZE_OF_MU);
   ...
   clear_elements(u, CYVA_SECTORS);
   clear_elements(mu, CYVA_SECTORS);
   clear_elements(v, q);
...
int CustodyUtils::create_custody_data(path content, uint32_t &n, char u_seed[], unsigned char pubKey[]) {
   ...
   char buffer[CYVA_SIZE_OF_POINT_ON_CURVE_COMPRESSED];
   for( int i = 0; i < n; i++ ) {
      element_to_bytes_compressed((unsigned char *) buffer, sigmas[i]);
      outfile.write(buffer, CYVA_SIZE_OF_POINT_ON_CURVE_COMPRESSED);
   }
...
int CustodyUtils::create_proof_of_custody(path content, const uint32_t n, const char u_seed[], unsigned char pubKey[],
   ...
   for( int i = 0; i < n; i++ ) {
      ...
      cusfile.read(&buffer[0], CYVA_SIZE_OF_POINT_ON_CURVE_COMPRESSED);
//      if(cusfile.readsome(buffer, CYVA_SIZE_OF_POINT_ON_CURVE_COMPRESSED) != CYVA_SIZE_OF_POINT_ON_CURVE_COMPRESSED )
//         return -9;
   ...
   mus.clear();
   //TODO_CYVA change the following code to store in hexadecimal string
   unsigned char buffer_mu[CYVA_SIZE_OF_MU];

   for( int i = 0; i < CYVA_SECTORS; i++ ) {
      memset(buffer_mu, 0, CYVA_SIZE_OF_MU);
      element_to_bytes(buffer_mu, mu[i]);
      std::string s = bytes_to_string(buffer_mu, CYVA_SIZE_OF_MU);
      mus.push_back(s);
   }
