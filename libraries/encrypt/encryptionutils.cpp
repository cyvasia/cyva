...
namespace cyva{ namespace encrypt{
...
{
    CryptoPP::Integer im (rng, CryptoPP::Integer::One(), CYVA_EL_GAMAL_MODULUS_512 -1);
    DInteger key = im;
    return key;
}
...
DInteger generate_private_el_gamal_key_from_secret(fc::sha256 secret)
{
   ...
   DInteger ret = key.Modulo( CYVA_EL_GAMAL_MODULUS_512 );
   return key;
}

DInteger get_public_el_gamal_key(const DInteger &privateKey)
{
    ModularArithmetic ma(CYVA_EL_GAMAL_MODULUS_512);
    DInteger publicKey = ma.Exponentiate(CYVA_EL_GAMAL_GROUP_GENERATOR, privateKey);
...
encryption_results el_gamal_encrypt(const point &message, const DInteger &publicKey, Ciphertext &result)
{
    ...
    key.AccessGroupParameters().Initialize(CYVA_EL_GAMAL_MODULUS_512, CYVA_EL_GAMAL_GROUP_GENERATOR);
    key.SetPublicElement(publicKey);

    CryptoPP::Integer randomizer(rng, CryptoPP::Integer::One(), CYVA_EL_GAMAL_MODULUS_512 - 1);

    try{
        byte buffer[CYVA_EL_GAMAL_GROUP_ELEMENT_SIZE];
        message.first.Encode(buffer, CYVA_MESSAGE_SIZE );
        message.second.Encode( buffer+CYVA_MESSAGE_SIZE, CYVA_MESSAGE_SIZE );
        SecByteBlock messageBytes(buffer, CYVA_EL_GAMAL_GROUP_ELEMENT_SIZE);

        ElGamal::GroupParameters groupParams;
        groupParams.Initialize(CYVA_EL_GAMAL_MODULUS_512, CYVA_EL_GAMAL_GROUP_GENERATOR);
        ModularArithmetic mr(CYVA_EL_GAMAL_MODULUS_512);

        CryptoPP::Integer m;
        m.Decode(messageBytes, CYVA_EL_GAMAL_GROUP_ELEMENT_SIZE);
...
encryption_results el_gamal_decrypt(const Ciphertext &input, const DInteger &privateKey, point &plaintext)
{
    ...
    try{
        ...
        byte recovered[CYVA_EL_GAMAL_GROUP_ELEMENT_SIZE];
        ElGamal::GroupParameters groupParams;
        groupParams.Initialize(CYVA_EL_GAMAL_MODULUS_512, CYVA_EL_GAMAL_GROUP_GENERATOR);
        ...
        m.Encode(recovered, CYVA_EL_GAMAL_GROUP_ELEMENT_SIZE);

        plaintext.first.Decode( recovered, CYVA_MESSAGE_SIZE );
        plaintext.second.Decode( recovered+CYVA_MESSAGE_SIZE, CYVA_MESSAGE_SIZE  );
...
DInteger hash_elements(Ciphertext t1, Ciphertext t2, DInteger key1, DInteger key2, DInteger G1, DInteger G2, DInteger G3)
{
   ...
   size_t hashSpace =9*(CYVA_EL_GAMAL_GROUP_ELEMENT_SIZE);
   hashier.CreateUpdateSpace(hashSpace);
   byte tmp[CYVA_EL_GAMAL_GROUP_ELEMENT_SIZE];

   t1.D1.Encode(tmp,CYVA_EL_GAMAL_GROUP_ELEMENT_SIZE);
   hashier.Update(tmp, CYVA_EL_GAMAL_GROUP_ELEMENT_SIZE);

   t1.C1.Encode(tmp,CYVA_EL_GAMAL_GROUP_ELEMENT_SIZE);
   hashier.Update(tmp, CYVA_EL_GAMAL_GROUP_ELEMENT_SIZE);

   t2.D1.Encode(tmp,CYVA_EL_GAMAL_GROUP_ELEMENT_SIZE);
   hashier.Update(tmp, CYVA_EL_GAMAL_GROUP_ELEMENT_SIZE);

   t2.C1.Encode(tmp,CYVA_EL_GAMAL_GROUP_ELEMENT_SIZE);
   hashier.Update(tmp, CYVA_EL_GAMAL_GROUP_ELEMENT_SIZE);

   key1.Encode(tmp,CYVA_EL_GAMAL_GROUP_ELEMENT_SIZE);
   hashier.Update(tmp, CYVA_EL_GAMAL_GROUP_ELEMENT_SIZE);

   key2.Encode(tmp,CYVA_EL_GAMAL_GROUP_ELEMENT_SIZE);
   hashier.Update(tmp, CYVA_EL_GAMAL_GROUP_ELEMENT_SIZE);

   G1.Encode(tmp,CYVA_EL_GAMAL_GROUP_ELEMENT_SIZE);
   hashier.Update(tmp, CYVA_EL_GAMAL_GROUP_ELEMENT_SIZE);

   G2.Encode(tmp,CYVA_EL_GAMAL_GROUP_ELEMENT_SIZE);
   hashier.Update(tmp, CYVA_EL_GAMAL_GROUP_ELEMENT_SIZE);

   G3.Encode(tmp,CYVA_EL_GAMAL_GROUP_ELEMENT_SIZE);
   hashier.Update(tmp, CYVA_EL_GAMAL_GROUP_ELEMENT_SIZE);
                      ...
                      const DInteger &firstPulicKey, const DInteger &secondPublicKey)
{
   ...
   key1.AccessGroupParameters().Initialize(CYVA_EL_GAMAL_MODULUS_512, CYVA_EL_GAMAL_GROUP_GENERATOR);
   key1.SetPublicElement(firstPulicKey);

   ElGamalKeys::PublicKey key2;
   key2.AccessGroupParameters().Initialize(CYVA_EL_GAMAL_MODULUS_512, CYVA_EL_GAMAL_GROUP_GENERATOR);
   ...
   DInteger t1 = mr.Exponentiate(CYVA_EL_GAMAL_GROUP_GENERATOR, proof.s);
                   ...
                   const Ciphertext &incoming, Ciphertext &outgoing, DeliveryProof &proof)
{
   ...
   try{
      ...
      private_key.AccessGroupParameters().Initialize(CYVA_EL_GAMAL_MODULUS_512, CYVA_EL_GAMAL_GROUP_GENERATOR);
      private_key.SetPrivateExponent(privateKey);

      ElGamalKeys::PublicKey public_key;
      public_key.AccessGroupParameters().Initialize(CYVA_EL_GAMAL_MODULUS_512, CYVA_EL_GAMAL_GROUP_GENERATOR);
      public_key.SetPublicElement(destinationPublicKey);


      CryptoPP::Integer randomizer(rng, CryptoPP::Integer::One(), CYVA_EL_GAMAL_MODULUS_512 - 1);

      byte messageBytes[CYVA_EL_GAMAL_GROUP_ELEMENT_SIZE];
      message.first.Encode(messageBytes, CYVA_MESSAGE_SIZE );
      message.second.Encode( messageBytes+CYVA_MESSAGE_SIZE, CYVA_MESSAGE_SIZE );
...
void ShamirSecret::calculate_split()
{
   ...
   ModularArithmetic mr(CYVA_SHAMIR_ORDER);
   for(int i=1; i<quorum; i++)
   {
      CryptoPP::Integer a(rng, CryptoPP::Integer::One(), CYVA_SHAMIR_ORDER);
      coef.push_back(a);
   }
