...
This is a quick introduction to get new developers up to speed on CYVA.


Building CYVA [![Build Status](https://travis-ci.com/CYVAnet/CYVA-net.svg?token=xwFm8bxNLqiJV3NaNYgy&branch=develop)](https://travis-ci.com/CYVAnet/CYVA-net)
---------------
...
(Ubuntu 14.04 LTS and 14.10 only; and only boost part for Ubuntu 16.04 LTS case) Note, that the default versions of GCC, CMake, and Boost installed in Ubuntu 14.04 LTS or 14.10 are too old and not supported. In order to install and use the supported ones, in addition to the common commands above, execute the following in console (in the same shell session, where you are going to build CYVA itself):
    ...
    # Download and build CMake 3.7.2
     mkdir -p ~/dev/CYVA/third-party
     cd ~/dev/CYVA/third-party
     rm -rf cmake-3.7.2*
...
### Obtaining the sources, building, and installing CYVA in Unix (macOS or Linux)

After all the prerequisites are installed, execute the following commands in console, in order to clone the repo, build, and install/stage CYVA:

    # Clone the repo.
     cd ~/
     git clone git@github.com:CYVAnet/CYVA-Initial.git
     cd CYVA-Initial
     git submodule update --init --recursive

    # Build and install CYVA.
     mkdir -p ~/CYVA-Initial/build
     cd ~/CYVA-Initial/build
...
By this time you should have CYVA files installed at `~/CYVA-Initial/build/artifacts/prefix` directory. You can specify any other custom install prefix for `cmake` during the initial configuration, for example, by adding `-D CMAKE_INSTALL_PREFIX=~/CYVA/prefix` to the command line.

You can use any path instead of `~/CYVA-Initial` in the steps above.
...
### Installing prerequisites, obtaining the sources, building, and installing CYVA in Windows
...
> In the commands below, change `~/dev/CYVAnet/CYVA-Initial/build/artifacts/prefix` to `~/dev/CYVAnet/CYVA-net-prefix` or to any other install location, that you specified during initial configuration.

On first run `cyvad` will create `.cyva` in the home directory, if doesn't exist already.

    $ ~/dev/CYVAnet/CYVA-net-build/artifacts/prefix/bin/cyvad

Optionally, now press Ctrl-C to stop `cyvad`. You can edit configuration in `~/.cyva/data/cyvad/config.ini`.

Then, run the cyva daemon again:

    $ ~/dev/CYVAnet/CYVA-net-build/artifacts/prefix/bin/cyvad
    
This will launch the cyva daemon node with the default genesis.

Then, in a separate console, start the command-line wallet by executing:

    $ cd ~/dev/CYVAnet/CYVA-net-working-dir
    $ ~/dev/CYVAnet/CYVA-net-build/artifacts/prefix/bin/cli_wallet
...
The role of the CYVA daemon is to broadcast transactions, download blocks, and optionally sign them.

    $ ~/dev/CYVAnet/CYVA-net-build/artifacts/prefix/bin/cyvad --rpc-endpoint 127.0.0.1:8090 --enable-stale-production -w '"1.4.0"'
...
2. Add parameters to the cyva daemon
...
When running `cyvad`, initially two API's are available:
...
- How do I get the `network_add_nodes` command to work?  Why is it so complicated?
    ...
    publicly accessible.  It's fine if your `cyvad` allows the general public to query
    the database or broadcast transactions (in fact, this is how the hosted web UI works).  It's
    less fine if your `cyvad` allows the general public to control which p2p nodes it's
