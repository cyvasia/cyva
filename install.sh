...
mkdir -p ~/dev/CYVAnet/CYVA-net-third-party
cd ~/dev/CYVAnet/CYVA-net-third-party
rm -rf cmake-3.7.2*
...
# Clone the repo.
 mkdir -p ~/dev/CYVAnet
 cd ~/dev/CYVAnet
 git clone https://github.com/CYVAnet/CYVA-ICO.git
 cd CYVA-ICO
 git submodule update --init --recursive

# Build and install CYVA.
 mkdir -p ~/dev/CYVAnet/CYVA-ICO-build
 cd ~/dev/CYVAnet/CYVA-ICO-build
 cmake -G "Unix Makefiles" -D CMAKE_BUILD_TYPE=Debug ../CYVA-ICO
 cmake --build . --target all -- -j -l 3.0
 cmake --build . --target install
 
mkdir ~/cyva
cp ~/dev/CYVAnet/CYVA-ICO-build/artifacts/prefix/bin/* ~/cyva/
cd ~/cyva
ls
echo "all done!"
